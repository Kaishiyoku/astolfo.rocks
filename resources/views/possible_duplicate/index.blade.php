                ...
                <a href="{{ route('possible_duplicates.show', $possibleDuplicate) }}" class="block transition sm:rounded-lg hover:opacity-70">
                    <x-card.card class="h-full overflow-hidden">
                        <x-card.body class="flex h-full transform hover:scale-110 transition ease-in-out">
                            <img src="{{ $possibleDuplicate->imageLeft->getThumbnailUrl() }}" alt="{{ $possibleDuplicate->imageLeft->id }}" class="max-w-[49%] h-full object-cover mr-[2%]" loading="lazy"/>
                            <img src="{{ $possibleDuplicate->imageRight->getThumbnailUrl() }}" alt="{{ $possibleDuplicate->imageRight->id }}" class="max-w-[49%] h-full object-cover" loading="lazy"/>
                        </x-card.body>
                    </x-card.card>
                </a>
            @endforeach
